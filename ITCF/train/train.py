...
if __name__ == '__main__':
    ...
    with open(path_config, 'r', encoding='utf-8') as f:
        config = f.read()
    config = yaml.load(config, yaml.FullLoader)['ITCF']

    # create model
    model = ITCF(config, data_target, data_source)
    # train
    path_log = '../res/log/'
    ...
    # save
    model_file = open("../res/model/ITCF_model.pkl", "wb")
    pickle.dump(model, model_file)
    model_file.close()

    # load model
    model_file = open("../res/model/ITCF_model.pkl", "rb")
