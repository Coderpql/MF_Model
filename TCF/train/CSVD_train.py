...
if __name__ == '__main__':
    ...
    with open(path_config, 'r', encoding='utf-8') as f:
        config = f.read()
    config = yaml.load(config, yaml.FullLoader)['CSVD']

    # create model
    model = CSVD(config, data_target, data_source)
    # train
    path_log = '../res/log/CSVD/'
    model.fit(path_log, data_test)
    # test
    mae, rmse = model.test(data_test)
    print('Test  mae: {:.4f}, rmse: {:.4f}'.format(mae, rmse))
    # save
    model_file = open("../res/model/CSVD_model.pkl", "wb")
    pickle.dump(model, model_file)
    model_file.close()

    # load model
    model_file = open("../res/model/CSVD_model.pkl", "rb")
